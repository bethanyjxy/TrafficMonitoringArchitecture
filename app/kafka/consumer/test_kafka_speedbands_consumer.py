...
from kafka_speedbands_consumer import handle_speedbands_message  # Adjust this import if needed
import json

class TestKafkaSpeedbandsConsumer(unittest.TestCase):

    @patch('kafka_to_hdfs.send_to_hdfs')
    def test_handle_speedbands_message(self, mock_send_to_hdfs):
        # Mock message
        data = {'speed_limit': 55, 'location': 'Main St & 1st Ave'}
        message = MagicMock()
        message.value.return_value = json.dumps(data).encode('utf-8')

        # Call the function
        handle_speedbands_message(message)

        # Assertions
        # mock_send_to_hdfs.assert_called_once_with('traffic_speedbands', data)
        # print(f"Handled speedband message: {data}")
    ...
    def test_consumer_loop(self, mock_initialize_consumer, mock_handle_errors, mock_commit_offsets):
        ...
        # Mock message
        message = MagicMock()
        message.value.return_value = json.dumps({'speed_limit': 55, 'location': 'Main St & 1st Ave'}).encode('utf-8')
        ...
        if msg is not None and mock_handle_errors(msg):
            handle_speedbands_message(msg)  # This should call send_to_hdfs
            mock_commit_offsets(consumer)

        # Assertions
        mock_initialize_consumer.assert_called_once_with('traffic_speedbands')
        mock_commit_offsets.assert_called_once_with(consumer)

        # Check if send_to_hdfs was called
        # expected_data = {'speed_limit': 55, 'location': 'Main St & 1st Ave'}
        # mock_send_to_hdfs.assert_called_once_with('traffic_speedbands', expected_data)
